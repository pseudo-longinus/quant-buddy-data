...
|---|---|
| `assets` | 用户提到的资产（≤1000） |
| `fields` | 参照 fast-snapshot.md 字段映射表（行情字段；估值字段 `PE_TTM`/`PB`/`PS_TTM`/`股息率`/`PCF` 及单季版 `PE_单季`/`PB_单季`/`PS_单季`/`股息率_单季` 在 window 模式同样支持 A/US/HK；`总市值`/`流通市值`/`换手率` 仅 A 股；A 股资金流向（主力/超大单/大单/中单/小单、北向持股）与港股南向持股、商品现货价格/库存亦可走 window 日频序列；期货仅尝试行情字段：收盘价、开盘价、最高价、最低价、涨跌幅、成交额、成交量） |
