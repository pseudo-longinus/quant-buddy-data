...
|------|------|------|
| A股个股（沪深主板/创业板/科创板/北交所） | ETF / LOF / 场外基金（先 grep 本地资产库，能唯一命中则正常执行；未命中才告知不支持）；期货行情/现货价格/库存（先 grep `presets/assets_db/future.yaml`，唯一命中后按工具返回尝试行情/现货/库存字段，库存单位为推测值；不承诺估值/财务/K线图） | 期权 |
...
> - **估值类**：
>   - A/US/HK：`PE`/`PE_TTM`/`PB`/`PS_TTM`/`股息率`/`PCF`/`PCF_现金净流量`（港美股使用 TTM〔估值数据〕，日频，服务端自动映射）
>   - 仅 A 股：`总市值`/`流通市值`/`换手率`（港美股数据源不准确，查询返回 `FIELD_MARKET_MISMATCH`）
>   - PE（静态）：A 股用静态 PE，港美股自动映射到 TTM 版
>   - 单季口径：`PE_单季`/`PB_单季`/`PS_单季`/`股息率_单季` 仍可用于显式查询季频数据
> - **财务类**（营业收入/净利润/归母净利润/营业成本/总资产/净资产/ROE/净利率/毛利率/现金流等）：A / HK / US 均支持（通过 `fast_query` 接口，统一返回单季数据）。
> - **资金流向 / 南北向持股**（主力资金各档位、北向持股仅 A 股；南向持股仅港股）、**商品期货**（行情/现货价格/库存，仅 A 股期货品种）：用 `fast_query` 的 `snapshot`/`window` 查询。
