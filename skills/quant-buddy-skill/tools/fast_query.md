...
适用：≤1000 资产，标准字段，行情/估值/财务/资金流向/南北向持股/商品标量、固定区间序列或窗口序列；期货支持行情/现货价格/库存字段，是否可得以工具返回为准。
...
**资金流向 / 南北向持股**（snapshot/window，股票×日期日频。`query_type` 用 `snapshot` 或 `window`，**不是 `report`**）：
- 仅 A 股：`主力资金净额`（`主力净额`/`主力资金净流入`/`主力资金流向`，万元）`主力资金净占比`（%）`超大单净额`/`超大单净占比` `大单净额`/`大单净占比` `中单净额`/`中单净占比` `小单净额`/`小单净占比`（万元 / %）；`北向持股比例`（`陆股通持股比例`，%）`北向持股市值`（`陆股通持股市值`，亿元）——2024-08 后北向转季频快照、非更新日为空值
- 仅港股：`南向持股比例`（`港股南向持股比例`，%）`南向持股市值`（`港股南向持股市值`，亿元，日频持续更新）
- A 股主力/北向字段查港股/美股返回 `FIELD_MARKET_MISMATCH`；港股南向字段查 A 股返回 `DATA_UNAVAILABLE`

**商品期货：现货价格 / 库存**（snapshot/window，仅 A 股期货品种，约 60 个品种，单位按品种走）：
- `现货价格`（`商品现货价格`/`现货价`）：约 50 品种有数据，单位多为元/吨
- `商品库存`（`库存`）`库存按发布日`（`商品库存按发布日`）：单位按品种**推测**（万吨/吨/千克/克/万重量箱/万立方米/头），响应带 `STOCK_UNIT_INFERRED` 警告，仅供参考
- 期货行情（收盘价/开盘价/最高价/最低价）已在「行情」白名单中，对期货资产单位按品种覆盖（元/吨、元/千克、元/克、元/桶、元/500千克、元/立方米）；成交额仍为元
- 期货品种用期货 ticker 查询即可命中（如 `RB.SHF`、`AG.SHF`）；查港股/美股返回 `FIELD_MARKET_MISMATCH`
- 期货**不承诺**估值/财务/K线字段
