...
> 它把资产识别、字段识别、数据读取和结果整理合成一次调用，覆盖 A 股、港股、美股和主要指数的大部分日常行情、估值、财务与历史序列需求；宏观一维序列可通过 `confirmDataMulti` 确认 `index_short_title` 短名称后读取。

如果你的需求是资产行情、估值、财务或历史序列下载，答案通常就是：**`fast_query`**；宏观一维序列走 `confirmDataMulti` + `readData`。
...
quant-buddy 面向日常下载和查数场景，覆盖常见资产、常见字段和常见时间范围。资产类行情、估值、财务优先用 `fast_query`；宏观一维序列先用 `confirmDataMulti` 确认 `index_short_title` 短名称。
...
| 宏观一维序列 | GDP、CPI、PPI、PMI、M1/M2、社融、信贷、外汇储备、国债收益率、LPR、OMO 利率、汇率等 | 先用 `confirmDataMulti` 确认 `index_short_title` 短名称，如 `中国GDP当季同比`、`中国CPI同比`、`USDCNH`，再用 `readData` 读取；具体以接口实际返回为准 |
...
| 宏观 | GDP、CPI、PPI、PMI、M1/M2、社融、信贷、外汇储备、国债收益率、LPR、OMO 利率、汇率等 | 多为一维时间序列，需按 `index_short_title` 短名称确认，适合宏观看板、策略背景变量、区间对比和本地建模 |
...
| 宏观一维序列，如 GDP/CPI/PPI/PMI/社融/利率/汇率 | `confirmDataMulti` 确认 `index_short_title` 短名称后用 `readData` 读取 |
