...
> 它把资产识别、字段识别、数据读取和结果整理合成一次调用，覆盖 A 股、港股、美股和主要指数的大部分日常行情、估值、财务与历史序列需求；宏观一维序列可通过 `confirmDataMulti` 确认数据名后读取。

如果你的需求是“我要下数据，该调什么接口”，答案通常就是：**`fast_query`**。
...
quant-buddy 面向日常下载和查数场景，覆盖常见资产、常见字段和常见时间范围。资产类行情、估值、财务优先用 `fast_query`；宏观一维序列先用 `confirmDataMulti` 确认数据名。
...
| 宏观一维序列 | GDP、CPI、PPI、PMI、M1/M2、社融、信贷、外汇储备、国债收益率、LPR、OMO 利率、美元兑人民币汇率、工业增加值、社零、进出口金额等 | 先用 `confirmDataMulti` 确认精确 `index_title`，再用 `readData` 读取；具体以接口实际返回为准 |
...
| 财务 | 营业收入、净利润、归母净利润、营业成本、总资产、净资产、ROE、净利率、毛利率、经营现金流 | `report` 查最近报告期；A/港/美财务字段以接口实际返回为准 |
| 派生字段 | 资产负债率 | 服务端可由总资产、净资产自动计算 |
| 宏观 | GDP 当季同比、CPI/PPI 当月同比、PMI、M1/M2 同比、新增社融、新增人民币贷款、社融存量、外汇储备、1/5/10 年国债收益率、LPR、美元兑人民币汇率等 | 多为一维时间序列，适合宏观看板、策略背景变量、区间对比和本地建模 |
| 下载格式 | JSON / CSV | 小结果直接返回 JSON；数据点超过 500 时自动切换 CSV 模式，返回 `csv_url` 下载链接 |

常用限制：单次最多 1000 个资产，窗口序列最多 2500 个交易日，单次最多 200,000 个数据点。日常个人研究、课程实验、看板取数和轻量批量下载通常够用。

本轮用 `confirmDataMulti(dimension="one-row")` 实测可匹配到的宏观数据包括：`GDP：不变价：当季同比`、`中国CPI：当月同比`、`中国PPI：全部工业品：当月同比`、`中国PMI`、`中国M1：同比`、`中国M2：同比`、`新增社融`、`社会融资规模存量`、`新增人民币贷款`、`比较：官方储备资产:外汇储备`、`中国1年期国债收益率`、`中国部分：中债5年期收益率`、`中国10年期国债收益率`、`贷款市场报价利率(LPR)：1年`、`OMO利率`、`美元兑人民币汇率`、`工业增加值:当月同比`、`中国:社会消费品零售总额:当月值`、`月度概况：进出口金额:当月值`。未命中的泛称或细项建议换更精确的中文名再确认。
...
| 宏观一维序列，如 GDP/CPI/PPI/PMI/社融/利率/汇率 | `confirmDataMulti` 确认 `index_title` 后用 `readData` 读取 |
